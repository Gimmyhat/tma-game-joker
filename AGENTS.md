...
1. **Прочитай `PROGRESS.md`** — текущий статус, что сделано, что в работе
2. **Найди секцию "Текущий фокус"** — определи приоритетные задачи
3. **Проверь Session Log** — что делали другие агенты

### При завершении сессии ОБЯЗАТЕЛЬНО:

1. **Обнови `PROGRESS.md`** — добавь запись в Session Log:
   ```markdown
...
2. **Закоммить изменения** (если были)
